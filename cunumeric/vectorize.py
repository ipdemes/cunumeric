...
class vectorize:
    def __init__(
        ...
        signature : string, optional
            ...
            WARNING: cuNumeric doesn't support this argument at the moment
        array_view : bool
            should array arguments be passed as a `view` to original array
            or not. The default value is `False` to be on-par with numpy,
            which will pass a copy of an input array to `vectorize` to prevent
            in-place updates. The `True` case will pass original arrays to the
            `vectorize` allowing in-lace updates and avoiding creating
            copies (e.g has better performance and saves memory)
        ...
        self._array_view: bool = array_view
    ...
    def _build_python_function(self) -> Callable[[Any], Any]:
        funcid = f"vectorized_{self._pyfunc.__name__}"
        # lines =[
        #    f"def {funcid}({_ARGS_VAR}):"]
        # lines.append("    import math")
        # lines.append("    import numpy")
        args = self._return_names + self._arg_names + self._scalar_names
        lines = ["def {}({}):".format(funcid, ",".join(args))]
        lines_old = self._func_body
        for line in lines_old:
            if not ("return" in line):
                lines.append(line)
        print("IRINA DEBUG lines ", lines)
        body = "\n".join(lines)
        glbs: Dict[str, Any] = {}
        exec(body, glbs)
        return glbs[funcid]

    def _get_numba_types(self) -> list[Any]:
        types = []
        for arg in self._return_args + self._args:
            type_a = arg.dtype
            type_a = str(type_a) if type_a != bool else "int8"
            type_a = getattr(numba.core.types, type_a)
            # FIXME I don't think we need cpointer here
    ...
    def _compile_func_cpu(self) -> numba.core.ccallback.CFunc:
        sig = numba.core.types.void(  # type: ignore
            # FIXME would need new numba wrpapper?    self._get_numba_types(),
    ...
    def _execute(self, is_gpu: bool, num_gpus: int = 0) -> None:
        if is_gpu and not self._created:
            self._create_cuda_function(num_gpus)
        ...
        if is_gpu:
            # adding 0 value for cpu_hash for consistency in the C++ code
    ...
    def _filter_arguments_and_check(self) -> None:
        ...
        # FIXME
        # we can potentially find common type of input arguments here and
        # broadcasted shapes to get output_dtype and output_shape
